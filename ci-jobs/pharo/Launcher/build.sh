...
DATE=$(date +%Y.%m.%d)
bash ./pharo-build-scripts/build-platform.sh -i One/Pharo -o Pharo -v $VERSION-$DATE -t Pharo -p mac
bash ./pharo-build-scripts/build-platform.sh -i One/Pharo -o Pharo -v $VERSION-$DATE -t Pharo -p win
